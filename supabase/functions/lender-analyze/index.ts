/**
 * GenTrack — lender-analyze Edge Function
 *
 * On-demand per-lender LLM advisory analysis via Gemini Flash Lite.
 * Caches result in lender_stats for 12 hours.
 *
 * POST { lender_name: string }
...
Deno.serve(async (req: Request) => {
  ...
  const { lender_name } = await req.json();
  if (!lender_name) {
    return new Response(JSON.stringify({ error: 'lender_name required' }), { status: 400 });
  }
  ...
  const { data: existing } = await sb
    .from('lender_stats')
    .select('analysis_text, analysis_angle_bullets, portfolio_synopsis, analysis_updated_at, asset_count, avg_plant_cf, pct_curtailed, facility_types, distress_score, news_sentiment_score')
  ...
  const { data: plantRows } = await sb
    .from('plant_lenders')
    .select('eia_plant_code, facility_type')
    .eq('lender_name', lender_name)
  ...
  interface PlantDetail { eia_plant_code: string; name: string; state: string; nameplate_capacity_mw: number; ttm_avg_factor: number | null; is_likely_curtailed: boolean; distress_score: number | null }
  ...
  if (plantCodes.length > 0) {
    const [plantsRes, statesRes] = await Promise.all([
      sb.from('plants')
        .select('eia_plant_code, name, state, nameplate_capacity_mw, ttm_avg_factor, is_likely_curtailed, distress_score')
  ...
  const plantDetails = plants
    ...
    .map(p => {
      ...
      return `- ${p.name} (${p.state}, ${p.nameplate_capacity_mw} MW, ${cf}${curtailed})${signal ? `\n  Signal: ${signal.slice(0, 200)}` : ''}`;
    }).join('\n');

  // ── Count underperforming (below 20% CF threshold for renewables or curtailed) ─
  ...
  // ── Build prompt ───────────────────────────────────────────────────────────
  const facilityTypes = (existing?.facility_types as string[] ?? []).join(', ') || 'N/A';
  const avgCf         = existing?.avg_plant_cf != null ? `${(Number(existing.avg_plant_cf) * 100).toFixed(1)}%` : 'N/A';
  ...
  const prompt = `You are an FTI Consulting power & utilities sector analyst generating an advisory briefing for a project finance lender.

LENDER: ${lender_name}
TOTAL ASSETS FINANCED: ${assetCount} plants
FACILITY TYPES: ${facilityTypes}
AVG PORTFOLIO PLANT CF (TTM): ${avgCf}
...
RECENT NEWS ARTICLES MENTIONING THIS LENDER (last 90 days):
${articleBlurbs || '(no recent articles found)'}

INSTRUCTIONS:
- Do NOT include specific dollar amounts, loan sizes, or transaction values in any output.
- Use asset counts, percentages, and directional language only (e.g., "significant portion", "majority of assets").
- Be factual and concise. Do not invent details not present in the data.

Generate three outputs:

1. analysis_text (2–3 sentences): Portfolio-level situation summary for this lender. Lead with total asset count and share underperforming. Cover concentration risk, curtailment exposure, and key news signals. No dollar amounts.

2. portfolio_synopsis: Two parts joined together:
   PART A — 2-3 sentence aggregate overview (asset count, % underperforming, tech/geo mix, overall risk pattern)
...
3. analysis_angle_bullets (3–5 bullets): Advisory angles FTI's Restructuring, Transactions, Disputes, or Policy teams could pursue.
...
Respond ONLY with valid JSON — no markdown fences, no extra keys:
{
  ...
  if (geminiKey) {
    try {
      ...
      if (geminiResp.ok) {
        ...
        console.error('[lender-analyze] Gemini error:', geminiResp.status, await geminiResp.text());
      }
    } catch (err) {
      console.error('[lender-analyze] Gemini parse error:', err);
    }
  }
  ...
  // ── Upsert cache ───────────────────────────────────────────────────────────
  await sb.from('lender_stats').upsert({
    lender_name,
