-- Patch v2 for auto_queue_lender_claims:
-- Fix "column reference lender_normalized is ambiguous" — OUT params collided
-- with column names inside the function body. Rename OUT params to out_*.
-- Apply via Supabase SQL Editor.

DROP FUNCTION IF EXISTS public.auto_queue_lender_claims(text);

CREATE FUNCTION public.auto_queue_lender_claims(p_plant_code text)
RETURNS TABLE (out_lender_normalized text, out_promoted boolean, out_lead_id bigint)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  FOR r IN
    SELECT
      c.lender_normalized                                         AS lender_normalized,
  ...
  LOOP
    ...
    IF NOT v_qualifies THEN
      out_lender_normalized := r.lender_normalized;
      out_promoted          := false;
      out_lead_id           := NULL;
      RETURN NEXT;
      CONTINUE;
    ...
    SELECT u.id INTO v_existing_id
     ...
     WHERE u.plant_code = p_plant_code
       AND u.lender_normalized = r.lender_normalized
     LIMIT 1;

    IF v_existing_id IS NOT NULL THEN
      UPDATE public.lender_evidence_claims c
